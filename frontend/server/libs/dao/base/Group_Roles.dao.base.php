...
abstract class GroupRolesDAOBase extends DAO
{
	/**
	  *	Campos de la tabla.
	  **/
	const FIELDS = '`Group_Roles`.`group_id`, `Group_Roles`.`role_id`, `Group_Roles`.`contest_id`';
	...
	{
		if (!is_null(self::getByPK( $Group_Roles->group_id, $Group_Roles->role_id, $Group_Roles->contest_id)))
		{
	...
	public static final function getByPK(  $group_id, $role_id, $contest_id )
	{
		...
		$sql = "SELECT `Group_Roles`.`group_id`, `Group_Roles`.`role_id`, `Group_Roles`.`contest_id` FROM Group_Roles WHERE (group_id = ? AND role_id = ? AND contest_id = ? ) LIMIT 1;";
	...
	{
		$sql = "SELECT `Group_Roles`.`group_id`, `Group_Roles`.`role_id`, `Group_Roles`.`contest_id` from Group_Roles";
		if( ! is_null ( $orden ) )
	...
	public static final function search( $Group_Roles , $orderBy = null, $orden = 'ASC', $offset = 0, $rowcount = NULL, $likeColumns = NULL)
	{
		...
		$sql = "SELECT `Group_Roles`.`group_id`, `Group_Roles`.`role_id`, `Group_Roles`.`contest_id` from Group_Roles WHERE (";
		$val = array();
		if (!is_null( $Group_Roles->group_id)) {
			$sql .= " `group_id` = ? AND";
			array_push( $val, $Group_Roles->group_id );
		}
		if (!is_null( $Group_Roles->role_id)) {
			$sql .= " `role_id` = ? AND";
			array_push( $val, $Group_Roles->role_id );
		}
		if (!is_null( $Group_Roles->contest_id)) {
			$sql .= " `contest_id` = ? AND";
			array_push( $val, $Group_Roles->contest_id );
		}
	...
	public static final function byRange( $Group_RolesA , $Group_RolesB , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( ( !is_null (($a = $Group_RolesA->group_id) ) ) & ( ! is_null ( ($b = $Group_RolesB->group_id) ) ) ){
		...
		if( ( !is_null (($a = $Group_RolesA->role_id) ) ) & ( ! is_null ( ($b = $Group_RolesB->role_id) ) ) ){
		...
		if( ( !is_null (($a = $Group_RolesA->contest_id) ) ) & ( ! is_null ( ($b = $Group_RolesB->contest_id) ) ) ){
	...
	{
		if( is_null( self::getByPK($Group_Roles->group_id, $Group_Roles->role_id, $Group_Roles->contest_id) ) ) throw new Exception('Campo no encontrado.');
		$sql = "DELETE FROM Group_Roles WHERE  group_id = ? AND role_id = ? AND contest_id = ?;";
		$params = array( $Group_Roles->group_id, $Group_Roles->role_id, $Group_Roles->contest_id );
		global $conn;
